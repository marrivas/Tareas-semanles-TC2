...
bp_sos_cheby = sig.iirdesign(wp=wp1 / nyq_frec, ws=ws1 / nyq_frec, gpass=0.5, gstop=40., analog=False, ftype='cheby1', output='sos')
bp_sos_cheby2 = sig.iirdesign(wp=wp2 / nyq_frec, ws=ws2 / nyq_frec, gpass=0.5, gstop=40., analog=False, ftype='cheby1', output='sos')
...
cheby_lp_y_hp = np.vstack([bp_sos_cheby,bp_sos_cheby2])
...
_, h_cheby_lp_y_hp = sig.sosfreqz(cheby_lp_y_hp, w_rad)
...
plt.plot(w, 20*np.log10(np.abs(h_cheby2)+1e-12), label='IIR-Cheby2 {:d}'.format(bp_sos_cheby2.shape[0]*2) )
plt.plot(w, 20*np.log10(np.abs(h_cheby_lp_y_hp)+1e-12), label='IIR-Cheby_lp_y_hp {:d}'.format(cheby_lp_y_hp.shape[0]*2) )
...
ECG_f_cheb = sig.sosfilt(bp_sos_cheby2, ecg_one_lead)
...
ECG_f_cheb = sig.sosfiltfilt(bp_sos_cheby2, ecg_one_lead)
